  ...
  <div class="register-page">
    ...
    <div class="register-container">
      <el-card class="register-card">
        <h2>Register</h2>
        <el-form :model="registerForm" :rules="dynamicRules" ref="registerForm" label-position="top">
          ...
          <el-form-item label="Confirm Password" prop="password_confirmation">
            <el-input type="password" v-model="registerForm.password_confirmation" placeholder="Confirm Password"></el-input>
          </el-form-item>
          <el-form-item label="Phone Number" prop="phone_number">
            <el-input v-model="registerForm.phone_number" placeholder="Phone Number"></el-input>
          </el-form-item>
          <el-form-item label="Region" prop="region_id">
            <el-select v-model="registerForm.region_id" filterable placeholder="Select Region" @change="fetchZones">
              <el-option v-for="region in regions" :key="region.id" :label="region.name" :value="region.id"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="Zone" prop="zone_id" v-if="zones.length">
            <el-select v-model="registerForm.zone_id" filterable placeholder="Select Zone" @change="fetchWoredas">
              <el-option v-for="zone in zones" :key="zone.id" :label="zone.name" :value="zone.id"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="Woreda" prop="woreda_id" v-if="woredas.length">
            <el-select v-model="registerForm.woreda_id" filterable placeholder="Select Woreda">
          ...
          <el-form-item label="Role" prop="role">
            <el-select v-model="registerForm.role" placeholder="Select Role">
              <el-option label="Region-Admin" value="region_admin"></el-option>
              <el-option label="Supervisor" value="supervisor"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item>
            <el-button type="primary" @click="handleRegister" class="submit-btn">Register</el-button>
          </el-form-item>
        </el-form>
        <div>
  ...
  data() {
    return {
      ...
      rules: {
        ...
        region_id: [{ required: true, message: 'Region is required', trigger: 'change' }],
  ...
  computed: {
    dynamicRules() {
      ...
      if (this.registerForm.role === 'supervisor') {
        rules.woreda_id = [{ required: true, message: 'Woreda is required for Supervisors', trigger: 'change' }];
      } else {
    ...
    async handleRegister() {
      try {
        ...
        if (error.response && error.response.data) {
          ...
          this.errorMessage = "Validation failed: " + JSON.stringify(error.response.data.errors);
        } else {
